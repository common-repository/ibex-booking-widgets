/**
 * Accommodation Single Property Booking Page
 */
var iBexBookingFrameController = {	
	 ...
	 */
	run: function(bookingFrameId, ibOperatorID, ibPropID, ibRU, ibMsgAgentURL, thisConfig){
		"use strict";
		...
		if(queryString.length > 0 && this.getURLParam('op').length > 0){
			...
			// If no PID is passed in, use the configuered one
			if(this.getURLParam('pid').length == 0){
				bookURL += '&pid=' + ibPropID;
			}
		}
		else {
			bookURL = 'http://ibex.seekom.com/accommodation/Property.php?reset=true&op=' + ibOperatorID + '&pid=' + ibPropID;
			if(ibRU != undefined && ibRU.length > 0){
				bookURL += '&ru=' + ibRU;
			}
			
			if(typeof thisConfig.rmtype !== 'undefined'){
				bookURL += '&rmtype=' + thisConfig.rmtype;
			}
...
jQuery(document).ready(function($) {
	...
	$('.ibw-shortcode.ibw-bookingpage').each(function(i,v){
		var bookingPageDiv = $(this),
			globalConfig = $('meta#ibw-config').data('config'),
			thisConfig = bookingPageDiv.data('ibexparams'),
			iFrameID = 'ibw-booking-' + (i + 1);
		
		$(this).append('<iframe id="' + iFrameID + '" class="bookingiFrame" onload="iBexFrame.SetHeight(this)" src="" width="' + thisConfig.width + '" height="' + thisConfig.height + '" scrolling="auto" style="border: none; overflow-y: auto;"></iframe>');
		
		var pid = (thisConfig.pid == 'auto') ? iBexBookingFrameController.getParam('pid') : thisConfig.pid;
		
		iBexBookingFrameController.run(iFrameID, globalConfig.operator, pid, thisConfig.ru, globalConfig.msgagent, thisConfig);
	});
});